					...
					<div class="inner">
						<div class="news-categories">
							...
							<?php if ($field = get_field('link_news', 'option')): ?>
								<a href="<?= $field['url'] ?>" class="news-category"<?php if($field['target']) echo ' target="_blank"' ?>><?= $field['title'] ?></a>
							<?php endif ?>

							<?php foreach ($terms as $term): ?>
								<a href="<?= get_term_link($term->term_id) ?>" class="news-category <?php if($category_id == $term->term_id) echo ' active' ?>"><?= $term->name ?></a>
							<?php endforeach ?>
		...
		<div class="page-layout-2col">
			<div class="column-left">
				<div class="post-list view-type-list" data-class="view-type-list">
					...
					<?php $wp_query = new WP_Query(array('post_type' => 'post', 'cat' => $category_id, 'paged' => get_query_var('paged')));
					$counter = 1;
			...
			<div class="column-right">
				<div class="recommended-list">
					<div class="inner">
						...
						<?php if ($field = get_field('recommended_title', 'term_' . $category_id)): ?>
							<h2 class="h1"><?= $field ?></h2>
						<?php endif ?>
						
						<?php
						$featured_posts = get_field('recommended_posts', 'term_' . $category_id);
						if($featured_posts): ?>
				...
				<?php if ($field = get_field('banner', 'term_' . $category_id)): ?>
					<div class="sidebar-banner"><?= $field ?></div>
				<?php endif ?>

				<?php if ($field = get_field('tweet', 'term_' . $category_id)): ?>
